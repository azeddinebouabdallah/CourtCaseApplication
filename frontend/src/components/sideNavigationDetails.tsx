import {
  ...
  class Navigation extends React.Component<IProps> {
    ...
    get currentComponent() {
      ...
      if (currentItem) {
        if (currentItem.headerText === "Attachments") {
          for (let i = 0; i < currentItem.component.props.items.length!; i++) {
            currentItem.component.props.items[i].url = <FabricButton text="Download"
              onClick={() => {
                window.location.href = currentItem.component.props.items[i].url
              }}
            />
          }
