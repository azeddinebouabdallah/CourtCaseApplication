...
export default function casePreviewData(store: RootStore): IPreview[] {
  const caseData: ICase = JSON.parse(store.getAllCases.caseData || "{}");
  console.log('tessssst', [caseData])
...
export function SingleCasePreviewData(store: RootStore): IPreview[] {
  const caseData: ICase = JSON.parse(store.getCase.singleCaseData || "{}");
  console.log('Single Case Data', [caseData])
  /* const testing:any = caseData;
  let stringic: string = testing.notes[0];
  const objNotes = JSON.parse(stringic)
  testing.notes = [];
